...
# 16 "./main.h" 2
    ...
    _Bool portb_prev[4];
    _Bool portb_enable[4];
    _Bool portb_send[4];
# 51 "main.c" 2
# 62 "main.c"
__attribute__((inline)) void disable_interrupts(void) {
    INTCONbits.GIE = 0;
}



__attribute__((inline)) void enable_interrupts(void) {
    INTCONbits.GIE = 1;
}



__attribute__((inline)) void enable_timer0() {
    INTCONbits.TMR0IE = 1;
    T0CONbits.TMR0ON = 1;
}
...
__attribute__((inline)) void disable_timer0() {
    INTCONbits.TMR0IE = 0;
    T0CONbits.TMR0ON = 0;
}
...
void portb_isr() {
    ...
    _delay((unsigned long)((1000)*(40000000/4000000.0)));
    ...
    if (portb_enable[0] && !current[0] && portb_prev[0]) {
        portb_send[0] = 1;
    }
    if (portb_enable[1] && !current[1] && portb_prev[1]) {
        portb_send[1] = 1;
    }
    if (portb_enable[2] && !current[2] && portb_prev[2]) {
        portb_send[2] = 1;
    }
    if (portb_enable[3] && !current[3] && portb_prev[3]) {
        portb_send[3] = 1;
...
void init_vars() {
    ...
    portb_prev[0] = portb_prev[1] = portb_prev[2] = portb_prev[3] = 0;
    portb_enable[0] = portb_enable[1] = portb_enable[2] = portb_enable[3] = 0;
    portb_send[0] = portb_send[1] = portb_send[2] = portb_send[3] = 0;
...
void parse() {
    ...
    while (!buf_isempty(INBUF)) {
        ...
        switch (parse_state) {
            ...
            case PARSE_BODY:
            {
                _Bool is_digit = (('0' <= value && value <= '9') || ('A' <= value && value <= 'F') || ('a' <= value && value <= 'f'));
                _Bool is_end = value == '#';
